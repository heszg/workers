...
public class MerchantOwnerScreen extends ScreenBase<MerchantInventoryContainer> {
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.drawString(font, playerInventory.getDisplayName().getVisualOrderText(), 8, imageHeight - 152 + 59, FONT_COLOR);

        if(limits != null && currentTrades != null){
            for (int i = 0; i < limits.size(); i++) {
                int limit = limits.get(i);
                if(limit != -1){
                    guiGraphics.drawString(font, currentTrades.get(i) +"/" + limits.get(i), 103,  21 + i * 18, FONT_COLOR);
                }
                else
                    guiGraphics.drawString(font, currentTrades.get(i) +"/" +"\u221E", 103,  21 + i * 18, FONT_COLOR);
            }
        }
    }
